...
recordForm.addEventListener('submit', async event => { event.preventDefault(); const payload = Object.fromEntries(new FormData(recordForm)); payload.published = recordForm.elements.published.checked; saveState.textContent = '正在保存…'; try { const id = payload.id; const record = await api(id ? `/api/admin/records/${id}` : '/api/admin/records', { method:id ? 'PUT' : 'POST', body:JSON.stringify(payload) }); await loadRecords(); formData(record); renderList(); saveState.textContent = '已保存'; } catch (error) { saveState.textContent = error.message; } });
deleteButton.addEventListener('click', async () => { const id = recordForm.elements.id.value; if (!id || !confirm('确定删除这则记录吗？此操作不可撤销。')) return; try { await api(`/api/admin/records/${id}`, { method:'DELETE' }); await loadRecords(); formData(records[0]); renderList(); } catch (error) { saveState.textContent = error.message; } });
